...
public class ProductSearchPanel extends JPanel {
	...
	public ProductSearchPanel() {
		...
		productSearchPanel.setBackground(SystemColor.menu);
		productSearchPanel.setPreferredSize(new Dimension(420, 750));
		scrollPane.setViewportView(productSearchPanel);
		productSearchPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 5, 5));
		
		JPanel searchListPanel = new JPanel();
		searchListPanel.setBackground(Color.WHITE);
		searchListPanel.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		searchListPanel.setFont(new Font("맑은 고딕", Font.PLAIN, 12));
		searchListPanel.setPreferredSize(new Dimension(420, 175));
		productSearchPanel.add(searchListPanel);
		...
		searchListPanel.add(bestSellerImageLabel);
		...
		searchListPanel.add(bestSellerNameLabel);
		
		JButton buyButton = new JButton("바로구매");
		buyButton.setBounds(294, 119, 80, 40);
		searchListPanel.add(buyButton);
		...
		searchListPanel.add(cartComboBox);
		...
		searchListPanel.add(cartAddButton);
		...
		searchListPanel.add(priceLabel);
	...
	public void displaySearchList(String name) throws Exception {
		List<Product> searchList = mainFrame.productService.searchProductByName(name);
		productSearchPanel.removeAll();
		...
		for (Product product : searchList) {
			...
			productSearchPanel.add(bestSellerListPanel);
